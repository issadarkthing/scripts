remote_id=$(
    xinput list | sed -n 's/.*USB Composite Device Keyboard.*id=\([0-9]*\).*keyboard.*/\1/p'
)
[ "$remote_id" ] || exit
...
xkb_symbols "remote" {
    key <AC01>  { [ XF86Tools ] };
    key <AE01>  { [ XF86AudioLowerVolume ] };
    key <AE03>  { [ XF86AudioRaiseVolume ] };
    key <AE04>  { [ XF86MonBrightnessDown ] };
    key <AE06>  { [ XF86MonBrightnessUp ] };
};
EOF
